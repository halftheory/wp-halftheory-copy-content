...
if ( ! class_exists('Halftheory_Copy_Content', false) && class_exists('Halftheory_Helper_Plugin', false) ) :
	...
	final class Halftheory_Copy_Content extends Halftheory_Helper_Plugin {
		...
		protected function setup_actions() {
			...
			if ( ! $this->is_public() ) {
				if ( ! class_exists('Halftheory_Copy_Content_Filters', false) && is_readable(__DIR__ . '/class-halftheory-copy-content-filters.php') ) {
					include_once __DIR__ . '/class-halftheory-copy-content-filters.php';
				}
		...
		public function menu_page() {
				...
				$save = function () use ( $plugin ) {
					...
					foreach ( array_keys($plugin->get_options_context('default')) as $value ) {
						...
						if ( in_array($value, $text_fields, true) ) {
							$_POST[ $name ] = trim(wp_unslash($_POST[ $name ]));
						}
			...
			<?php foreach ( $plugin->shortcodes as $shortcode => $shortcode_prefix ) : ?>
				...
				<div class="postbox">
					<div class="inside">
						<p><label for="<?php echo esc_attr($plugin::$prefix); ?>_<?php echo esc_attr($shortcode_prefix); ?>_update_excerpt"><input type="checkbox" id="<?php echo esc_attr($plugin::$prefix); ?>_<?php echo esc_attr($shortcode_prefix); ?>_update_excerpt" name="<?php echo esc_attr($plugin::$prefix); ?>_<?php echo esc_attr($shortcode_prefix); ?>_update_excerpt" value="1"<?php checked($options[ $shortcode_prefix . '_update_excerpt' ], true); ?> /> <?php esc_html_e('Update my post <strong>excerpt</strong> where possible.'); ?></label></p>

						<p><label for="<?php echo esc_attr($plugin::$prefix); ?>_<?php echo esc_attr($shortcode_prefix); ?>_update_thumbnail"><input type="checkbox" id="<?php echo esc_attr($plugin::$prefix); ?>_<?php echo esc_attr($shortcode_prefix); ?>_update_thumbnail" name="<?php echo esc_attr($plugin::$prefix); ?>_<?php echo esc_attr($shortcode_prefix); ?>_update_thumbnail" value="1"<?php checked($options[ $shortcode_prefix . '_update_thumbnail' ], true); ?> /> <?php esc_html_e('Update my post <strong>thumbnail</strong> where possible.'); ?></label></p>
		...
		public function menu_page_transients( $plugin ) {
			...
			if ( $plugin->save_menu_page(__FUNCTION__) ) {
				$save = function () use ( $plugin ) {
					...
					foreach ( $input_arr as $value ) {
						...
						$input[ $value ] = wp_unslash($_POST[ $name ]);
					}
						...
						if ( ! empty($_POST[ $name ]) && is_array($_POST[ $name ]) ) {
							...
							foreach ( $_POST[ $name ] as $key => $value ) {
								...
								if ( $str = $plugin->get_transient($value) ) {
									$url = isset($_POST[ $plugin::$prefix . '_url_' . $value ]) ? trim(wp_unslash($_POST[ $plugin::$prefix . '_url_' . $value ])) : '';
			...
			<div class="postbox">
				<div class="inside">
					<label for="<?php echo esc_attr($plugin::$prefix); ?>_url">
						...
						<?php
						$val = isset($_POST[ $plugin::$prefix . '_url' ]) ? sanitize_url(wp_unslash($_POST[ $plugin::$prefix . '_url' ])) : '';
						?>
					...
					<label for="<?php echo esc_attr($plugin::$prefix); ?>_content">
						...
						<?php
						$val = isset($_POST[ $plugin::$prefix . '_content' ]) ? wp_unslash($_POST[ $plugin::$prefix . '_content' ]) : '';
						?>
			...
			$items = $wpdb->get_col($wpdb->prepare("SELECT option_name FROM $wpdb->options WHERE option_name LIKE %s ORDER BY option_id ASC", $wpdb->esc_like('_transient_' . $plugin::$prefix . '_') . '%'));
			if ( ! empty($items) ) {
				// include table.
				if ( ! class_exists('Halftheory_Copy_Content_Table_Transients', false) && is_readable(__DIR__ . '/class-halftheory-copy-content-table-transients.php') ) {
					include_once __DIR__ . '/class-halftheory-copy-content-table-transients.php';
				}
				if ( class_exists('Halftheory_Copy_Content_Table_Transients', false) ) {
					...
					$arr = $wpdb->get_results($wpdb->prepare("SELECT ID,post_content FROM $wpdb->posts WHERE post_content LIKE %s AND post_content LIKE %s AND post_type != 'revision' AND post_status != 'auto-draft' ORDER BY ID ASC", '%[' . $wpdb->esc_like($plugin->shortcode_copycontent) . ' %', '% url=%'));
					if ( ! empty($arr) ) {
		...
		public function meta_box( $post, $args ) {
			...
			foreach ( $urls as $url ) {
				if ( $v = $this->get_transient_from_url($url) ) {
					...
					?>
					<li>
						<p><?php echo wp_kses_post(wptexturize(implode(__(' - '), $arr))); ?></p>
		...
		public function loop_end( $wp_query ) {
			...
			if ( isset($this->loop_ends) && ! empty($this->loop_ends) ) {
			...
			if ( ! empty($tags) ) {
				...
				foreach ( $tags as $key => $value ) {
					if ( is_numeric($value) || is_bool($value) || $value === 'true' || $value === 'false' ) {
						...
						continue;
					} elseif ( ! array_key_exists($key, $arr['attr']) ) {
		...
		private function loadHTML( $str ) {
			...
			$dom->loadHTML('<' . $this->domwrapper . '>' . htmlentities($str, ENT_COMPAT, 'UTF-8') . '</' . $this->domwrapper . '>', LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);
